...
getpw(void) {
  ...
  if (!pw) {
    if (errno)
      die("takkun: getpwuid: %s\n", strerror(errno));
    else
      die("takkun: cannot retrieve password entry\n");
  }
  ...
  if (rval[0] == 'x' && rval[1] == '\0') {
    ...
    if (!sp)
      die("takkun: cannot retrieve shadow entry\n");
    endspent();
    rval = sp->sp_pwdp;
  }
#endif
  ...
  if (geteuid() == 0) {
    if (!(getegid() != pw->pw_gid && setgid(pw->pw_gid) < 0)) {
      if (setuid(pw->pw_uid) < 0)
        die("takkun: cannot drop privileges\n");
    }
  }
...
webcam_shot(int async) {
  ...
  int r = snprintf(
    ...
    " -frames:v 1 -f image2 %s/takkun.jpg%s",
    getenv("HOME"),
    async ? " &" : ""
  );
...
imgur_upload(char **link, char **hash) {
  ...
  *hash = NULL;
  ...
  r = snprintf(
    ...
    " -F 'image=@%s/takkun.jpg'"
    " 'https://api.imgur.com/3/image' > %s/takkun_imgur.curl",
    HOME,
    HOME
  );
  ...
  r = snprintf(cmd, CMD_LENGTH, "%s/takkun_imgur.link", HOME);
  ...
  *link = read_file(cmd);
  ...
  r = snprintf(cmd, CMD_LENGTH, "%s/takkun_imgur.hash", HOME);
...
cleanup:
  r = snprintf(cmd, CMD_LENGTH, "%s/takkun_imgur.curl", HOME);
...
play_beep(int async) {
  ...
  int r = snprintf(
    cmd,
    CMD_LENGTH,
    "aplay %s/takkun/beep.wav 2> /dev/null%s",
    getenv("HOME"),
    async ? " &" : ""
  );
...
play_alarm(int async) {
  ...
  int r = snprintf(
    cmd,
    CMD_LENGTH,
    "aplay %s/takkun/police.wav 2> /dev/null%s",
    getenv("HOME"),
    async ? " &" : ""
  );
...
#endif
{
  ...
  // you can set it with "xset" or some other
...
lockscreen(Display *dpy, int screen) {
  ...
  XTextProperty name_prop = { "takkun", name_atom, 8, 5 };
  XSetWMName(dpy, lock->win, &name_prop);

  XClassHint *hint = XAllocClassHint();
  if (hint) {
    hint->res_name = "takkun";
    hint->res_class = "takkun";
...
main(int argc, char **argv) {
  ...
  if (!g_pw && !getpwuid(getuid()))
    die("takkun: no passwd entry for you\n");
  ...
  dpy = XOpenDisplay(0);
  if (!dpy)
    die("takkun: cannot open display\n");
